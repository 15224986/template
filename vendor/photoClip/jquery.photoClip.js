 ...
 *
 * @author 赵世杰 15224986@qq.com 2018/9/12
 * @effect 添加了放大按钮、缩小按钮、旋转按钮、最大缩放比例
 *
 * @brief	支持手势的裁图插件
...
(function(root, factory) {
	...
	function photoClip(container, option) {
		...
		$(bigBtn).click(function(){
			if(zoomStartNum < option.zoomMax*4){
				zoomStartNum++;
			}	
			var sf = zoomStartNum * .25;
			myScroll.zoom(sf);
		});
		...
		function resetScroll() {
			...
			// 点击放大缩小按钮的初始化赋值
			zoomStartNum = myScroll.options.zoomStart*4;
		...
		function calculateScale(width, height) {
			...
			myScroll.options.zoomMax = Math.max(option.zoomMax, myScroll.options.zoomMin);
			myScroll.options.zoomStart = Math.min(1, getScale(containerWidth, containerHeight, width, height));
		}
		...
		function init() {
			...
			// 创建遮罩
			var $mask = $("<div class='photo-clip-mask'>").appendTo($container);
