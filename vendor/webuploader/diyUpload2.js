...
(function( $ ) {
    ...
    $.fn.extend({
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			$fileInput.parent().addClass('WebUploader-data-box');
			...
			webUploader.on('uploadSuccess',function( file, response ){
                ...
                $fileBox.attr("data-src",response.filepath);
                $fileBox.attr("data-fileName",response.olefilename);
	...
	function getOption(objId) {
		...
		*/
		return {
            ...
            // 分片大小
            chunkSize:5242880,
            //最大上传的文件数量, 总文件大小,单个文件大小(单位mb);
            fileNumLimit:50,
            fileSizeLimit:5000 * 1024000,
            fileSingleSizeLimit:100 * 1024000
		};
	}
	...
	function getUploader( opt ) {
		var uploaderMyItem = new WebUploader.Uploader(opt);
        uploaderMy.push( uploaderMyItem );
		return uploaderMy[uploaderMy.length-1];
	}
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		//添加子容器;
		var li = '<li id="fileBox_'+file_id+'" class="fileBoxLi diyUploadHover"> \
					<div class="viewThumb"></div> \
					<div class="diyCancel"></div> \
					<div class="diySuccess"></div> \
					<div class="diyRemove"></div> \
...
(function( $ ) {
    ...
    $.fn.extend({
		...
		*	serverCallBack回调函数 每个文件上传至服务端后,服务端返回参数,无论成功失败都会调用 参数为服务器返回信息;
		*/
        fileUpload:function( opt, serverCallBack ) {
 			if ( typeof opt != "object" ) {
				alert('参数错误!');
				return;	
			}
			...
			var webUploader = getUploader( opt, $fileInput );
			...
			webUploader.on('uploadProgress',function( file, percentage  ){
				...
				$diyProgress.width(percentage).text(percentage);
			...
			webUploader.on('uploadSuccess',function( file, response ){
				...
				$fileBox.find('.diyType').text('上传成功').addClass('diyInfoSuccess');
				$fileBox.removeClass('diyUploadHover');
				$fileBox.attr("data-src",response.filepath);
				$fileBox.attr("data-fileName",response.olefilename);
				if ( successCallBack ) {
			...
			webUploader.on('uploadError',function( file, reason ){
				var $fileBox = $('#fileBox_'+file.id);
				$fileBox.find('.diyBar').hide();
				$fileBox.find('.diyType').text('上传失败').addClass('diyInfoCancel');
				var err = '上传失败! 文件:'+file.name+' 错误码:'+reason;
				if ( errorCallBack ) {
	...
	//实例化Web Uploader
	fileloaderMy = [];
    function getUploader( opt, $fileInput ) {
    	var fileloaderMyItem = new WebUploader.Uploader(opt);
    	// console.log( $fileInput.attr('id') );
        fileloaderMy.push( fileloaderMyItem );
        return fileloaderMy[fileloaderMy.length-1];
    }

	//取消事件;	
	function removeLi ( $li ,file_id ,webUploader) {
		webUploader.removeFile( file_id );
		if ( $li.siblings('.webuploader-list-item').length <= 0 ) {
			$li.parents('.parentFileBox').remove();
		} else {
			$li.remove();
		}
	...
	function createBox( $fileInput, file, webUploader, opt ) {
		...
		//创建按钮
		if ( $parentFileBox.find('.diyButton').length <= 0 && opt.auto == false ) {
		...
		//添加子容器;
		var li = '<div id="fileBox_'+file_id+'" class="webuploader-list-item diyUploadHover"> \
					<div class="webuploader-list-info"> \
						<span class="diyFileName">'+file.name+'</span> \
						<span class="diyType"></span> \
						<a class="diyRemove" href="javascript:;">删除</a> \
						<a class="diyCancel" href="javascript:;">删除</a> \
					</div> \
					<div class="diyBar"> \
		...
		var $diyCancel = $fileBox.find('.diyCancel').one('click',function(){
			removeLi( $(this).closest('.webuploader-list-item'), file_id, webUploader );	
		});
	}
