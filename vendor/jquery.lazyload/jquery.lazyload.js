...
(function($, window, document, undefined) {
    ...
    $.fn.lazyload = function(options) {
        ...
        var settings = {
            ...
            event           : "scroll",
            // effect,当图片完全加载的时候，插件默认地使用show()方法来将图显示出来。你也可以使用其他的效果有show(直接显示),fadeIn(淡入),slideDown(下拉)等,常用fadeIn
            // 如果想要控制动画的速度,还可以修改 effectspeed 属性. $("img.lazy").lazyload({ effect : "fadeIn", effectspeed : 1000 }); 
            effect          : "show",
            // container,值为某容器.lazyload默认在拉动浏览器滚动条时生效,这个参数可以让你在拉动某DIV的滚动条时依次加载其中的图片
            container       : window, // 对某容器中的图片实现效果
