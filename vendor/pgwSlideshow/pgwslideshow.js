...
;(function($){
    $.fn.pgwSlideshow = function(options) {
        ...
        var setup = function() {
            ...
            pgwSlideshow.plugin.find('.ps-list > ul > li').each(function(index,el) {
                ...
                // 缩略图添加结构等
                $(el).addClass('ps-list-item elt_' + element.id).attr('data-index',element.id);
                $(this).wrapInner('<span class="ps-item' + (elementId == 1 ? ' ps-selected' : '') + '"></span>');
                
                // 在大图里面插入html
                var currentElement = $('<li class="ps-current-item elt_' +  element.id + '"></li>');
                currentElement.html( currentHtml );
            ...
            if (pgwSlideshow.config.displayList) {
                ...
                pgwSlideshow.plugin.addClass('ps-navigation');
                pgwSlideshow.plugin.find('.ps-list .ps-list-item').each(function(index, el) {
                    var dataIndex = $(el).data("index");
                    $(el).find('span').text(dataIndex);
                });
            }
            // Attach slide events
            if (pgwSlideshow.config.autoSlide) {
